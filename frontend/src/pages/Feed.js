    ...
    <div style={{
      ...
      <div style={{
        ...
        <button onClick={handleSubmit} disabled={loading || (!content.trim() && !mediaFile)}
          ...
          onMouseEnter={e => { if (!loading) { e.currentTarget.style.background = 'var(--mint-dark)'; e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 6px 22px rgba(61,191,160,0.42)'; }}}
          onMouseLeave={e => { e.currentTarget.style.background = 'var(--mint)'; e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 4px 14px rgba(61,191,160,0.30)'; }}
        >
    ...
    <div className="comment-section-anim" style={{
      ...
      <div style={{ display: 'flex', gap: 9, alignItems: 'center' }}>
        ...
        <button onClick={submit} disabled={loading || !text.trim()}
          ...
          onMouseEnter={e => { if (!loading && text.trim()) { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 6px 18px rgba(61,191,160,0.38)'; }}}
          onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 4px 12px rgba(61,191,160,0.25)'; }}
        >Send</button>
      </div>
    </div>
  );
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 8, justifyContent: 'flex-end' }}>
        <button onClick={onCancel}
          style={{
            ...
            fontWeight: 600, cursor: 'pointer', color: 'var(--text-soft)',
        ...
        <button onClick={save} disabled={saving || !content.trim()}
          ...
          onMouseEnter={e => { if (!saving) { e.currentTarget.style.transform = 'translateY(-1px)'; e.currentTarget.style.boxShadow = '0 6px 18px rgba(61,191,160,0.42)'; }}}
          onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 4px 12px rgba(61,191,160,0.28)'; }}
        >
...
function PostCard({ post, user, onDeleted, index = 0 }) {
  ...
  const handleEditSaved = (newContent) => { setPostContent(newContent); setIsEditing(false); };
    ...
    <AnimatedCard delay={Math.min(index * 60, 300)}>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 11, marginBottom: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {isAuthor && (
              <button onClick={() => setIsEditing(v => !v)} title="Edit post"
                style={{
                  background: isEditing ? 'var(--mint-light)' : 'var(--surface2)',
                  border: `1px solid ${isEditing ? 'var(--mint)' : 'var(--border)'}`,
                  fontSize: 15, cursor: 'pointer', borderRadius: 10,
                  padding: '6px 10px', color: isEditing ? 'var(--mint)' : 'var(--text-muted)',
                  transition: 'all .2s',
                }}
                onMouseEnter={e => { if (!isEditing) { e.currentTarget.style.borderColor = 'var(--mint)'; e.currentTarget.style.color = 'var(--mint)'; e.currentTarget.style.transform = 'rotate(-8deg) scale(1.12)'; }}}
                onMouseLeave={e => { if (!isEditing) { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.color = 'var(--text-muted)'; e.currentTarget.style.transform = 'rotate(0deg) scale(1)'; }}}
              >✏️</button>
            )}
            {(isAuthor || user?.role === 'super_admin') && (
              <button onClick={handleDelete}
                style={{
                  ...
                  padding: '6px 10px', color: 'var(--text-muted)', transition: 'all .2s',
                }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = '#F4845F'; e.currentTarget.style.color = '#F4845F'; e.currentTarget.style.transform = 'scale(1.12)'; }}
        ...
        {}
        {isEditing
          ? <EditPostBox post={{ ...post, content: postContent }} onSaved={handleEditSaved} onCancel={() => setIsEditing(false)} />
          : postContent && (
              <p style={{
                fontSize: 15, lineHeight: 1.7, margin: '0 0 14px',
                color: 'var(--text-soft)', fontWeight: 400, fontFamily: "'DM Sans', sans-serif",
              }}>{postContent}</p>
            )
        }
        ...
        <div style={{
          ...
          borderTop: '1px solid var(--border)', marginTop: 4, position: 'relative'
        }}>
          {/* React */}
          <div style={{ flex: 1, position: 'relative' }}
            ...
            <button
              ...
              className={reactBounce ? 'react-bounce' : ''}
              onClick={() => handleReact(myReaction || 'like')}
              onMouseEnter={e => { if (!myReaction) { e.currentTarget.style.color = 'var(--mint)'; e.currentTarget.style.transform = 'translateY(-1px)'; }}}
              onMouseLeave={e => { if (!myReaction) { e.currentTarget.style.color = 'var(--text-soft)'; e.currentTarget.style.transform = 'translateY(0)'; }}}
            >
          ...
          <button style={actionBtn(showComments)} onClick={() => setShowComments(v => !v)}
            onMouseEnter={e => { if (!showComments) { e.currentTarget.style.color = 'var(--mint)'; e.currentTarget.style.transform = 'translateY(-1px)'; }}}
            onMouseLeave={e => { if (!showComments) { e.currentTarget.style.color = 'var(--text-soft)'; e.currentTarget.style.transform = 'translateY(0)'; }}}
          >💬 Comment</button>

          <button style={actionBtn(false)} onClick={handleShare} disabled={shareLoading}
            onMouseEnter={e => { e.currentTarget.style.color = 'var(--mint)'; e.currentTarget.style.transform = 'translateY(-1px)'; }}
            onMouseLeave={e => { e.currentTarget.style.color = 'var(--text-soft)'; e.currentTarget.style.transform = 'translateY(0)'; }}
          >
            {shareLoading ? <span className="posting-spinner">⟳</span> : '🔗'} Share
          </button>
        </div>
...
function SearchBar({ onResults, onClear }) {
  ...
  const handleClear = () => { setQ(''); clearTimeout(debounceRef.current); onClear(); };

  return (
    <div className={`search-bar${searching ? ' search-bar--active' : ''}`} style={{
      background: 'var(--card-bg)', borderRadius: 18, padding: '14px 18px',
      border: '1.5px solid var(--border)', backdropFilter: 'blur(20px)', boxShadow: 'var(--shadow)',
      display: 'flex', alignItems: 'center', gap: 10, transition: 'border-color .25s, box-shadow .25s',
    }}>
...
export default function Feed() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      console.log('[Feed] localStorage user:', parsed);
      setUser(parsed);
    }
  }, []);
    ...
    <>
      ...
      <div className="feed-container">
        <div className="feed-inner">
          ...
          {isSearching && (
            <div className="search-label-anim" style={{
              fontSize: 13, color: 'var(--text-muted)',
              fontFamily: "'DM Sans', sans-serif", paddingLeft: 4,
            }}>
          ...
          {loading && (
            <div style={{ textAlign: 'center', padding: 28 }}>
              <LoadingDots label="Loading posts" />
            </div>
          )}
...
const feedStyles = `
  ...
  .create-box-entrance {
    animation: createEntrance 0.52s cubic-bezier(.22,.68,0,1.2) both;
  }

  .post-card { will-change: transform, box-shadow; }

  .reaction-picker-anim {
    animation: popUp 1.2s cubic-bezier(.22,.68,0,1.35) both;
  ...
  .comment-section-anim { animation: fadeSlideDown 0.28s ease both; }

  .comment-item-anim {
    animation: commentBounceIn 0.34s cubic-bezier(.22,.68,0,1.2) both;
  }

  .edit-box-anim { animation: fadeSlideDown 0.24s ease both; }

  .who-reacted-anim  { animation: fadeSlideDown 0.24s ease both; }
  .who-reacted-row   { animation: fadeSlideUp  0.3s ease both;   }

  .media-fade-in  { animation: fadeIn 0.4s ease both; }
  .preview-fade-in{ animation: popIn  0.3s ease both; }

  .search-label-anim { animation: fadeIn 0.3s ease both; }
  .search-bar { transition: border-color .25s, box-shadow .25s; }
  .search-bar--active {
    border-color: var(--mint) !important;
    box-shadow: 0 0 0 3px rgba(61,191,160,0.13) !important;
  }
  .search-icon { display: inline-block; transition: transform .3s; }
  .search-icon--spin { animation: spinCW 1s linear infinite; }

  .empty-state-anim {
    animation: createEntrance 0.5s ease both, emptyFloat 3.2s ease 0.8s infinite;
  }
  ...
  .avatar-ring:hover { animation: avatarRingPulse 0.7s ease; }
  ...
  .dot2 { animation-delay: 160ms; }
  .dot3 { animation-delay: 320ms; }

  .posting-spinner { display: inline-block; animation: spinCW 0.7s linear infinite; }
  ...
  @media (max-width: 680px) {
    .feed-container { padding: 16px 10px; }
  }
`;
